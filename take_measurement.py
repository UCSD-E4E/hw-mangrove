...
take_measurement = Blueprint('take_measurement', __name__)
...
from take_measurement import MPL3115A2
mpl3115a2 = MPL3115A2()
...
"""
@take_measurement.route('/getTime', methods=['GET'])
    def getTime():
        clientTime = request.args.get("time")
        return clientTime
#trying again to get exact time
"""
date =1 
with open("logs/"+str(date)+".txt", 'a') as f:
        f.write("New data writing ...\n")
...
def takeMeasurement():
	...
	pres = mpl3115a2.read_pres()
        ...
        alt = mpl3115a2.read_alt_temp()
        pres = mpl3115a2.read_pres()
        now = request.get_data()
        #raise Exception("hi")
        print(str(now))
        with open("logs/"+str(date)+".txt", 'a+') as f:
            f.write("Altitude: "+str(format(alt['a'],'.5f'))
                    +"\tPressure: "+str(format(pres['p'],'.5f'))
